...
ENV_RABBITMQ_AUTO_ENABLE_ALL_FEATURE_FLAGS=${RABBITMQ_AUTO_ENABLE_ALL_FEATURE_FLAGS:-}
ENV_RABBITMQ_OPERATOR_IMAGE_PULL_POLICY=${RABBITMQ_OPERATOR_IMAGE_PULL_POLICY:-}
...
RABBITMQ_AUTO_ENABLE_ALL_FEATURE_FLAGS=${ENV_RABBITMQ_AUTO_ENABLE_ALL_FEATURE_FLAGS:-${RABBITMQ_AUTO_ENABLE_ALL_FEATURE_FLAGS:-false}}
RABBITMQ_OPERATOR_IMAGE_PULL_POLICY=${ENV_RABBITMQ_OPERATOR_IMAGE_PULL_POLICY:-${RABBITMQ_OPERATOR_IMAGE_PULL_POLICY:-IfNotPresent}}
...
if [[ "$RABBITMQ_INSTALL_OPERATOR" == "true" ]]; then
  ...
  if [[ -n "$RABBITMQ_OPERATOR_IMAGE_PULL_POLICY" ]]; then
    echo "设置 RabbitMQ Cluster Operator 镜像拉取策略: $RABBITMQ_OPERATOR_IMAGE_PULL_POLICY"
    kubectl -n rabbitmq-system patch deployment rabbitmq-cluster-operator --type=json \
      -p="[{\"op\":\"replace\",\"path\":\"/spec/template/spec/containers/0/imagePullPolicy\",\"value\":\"$RABBITMQ_OPERATOR_IMAGE_PULL_POLICY\"}]"
  fi
...
sed \
  ...
  -e "s|__RABBITMQ_AUTO_ENABLE_ALL_FEATURE_FLAGS__|$(escape_sed "$RABBITMQ_AUTO_ENABLE_ALL_FEATURE_FLAGS")|g" \
