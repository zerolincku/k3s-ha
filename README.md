...
当前已完成：3 台 master 节点的 K3s 高可用集群方案、Redis Sentinel 高可用部署方案、RabbitMQ 高可用部署方案，以及 Rook Ceph + CephFS + RGW 共享存储和对象存储方案。后续 MinIO、MySQL 等中间件高可用部署方案按同一目录约定扩展。
...
- Rook Ceph 共享存储与对象存储部署：[rook-ceph/说明.md](rook-ceph/说明.md)
- Rook Ceph OrbStack 验证记录：[rook-ceph/docs/05-OrbStack验证记录.md](rook-ceph/docs/05-OrbStack验证记录.md)
